...
const counterFactory = (function (defaultVal = 0) {
...
const myPow = (base, exp, myPrint) => {
  const calcPow = (b, e) => {
    if (e === 0) return 1;
    if (e === 1) return base;

    return b * calcPow(b, e - 1);
  };

  const absoluteExp = Math.abs(exp);
  const rawResult = calcPow(base, absoluteExp);
  const finalResult = exp < 0 ? 1 / rawResult : rawResult;

  return myPrint(base, exp, finalResult);
};
